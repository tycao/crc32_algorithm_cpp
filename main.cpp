...
////////////////////////////////////
uint CRC32_Fast__(byte bytes[], uint len)
{
    uint crc = 0;
    for (int i = 0; i < len; ++i)
    {
        ...
        byte pos = (byte)((crc ^ (b << 24)) >> 24);
        /* Shift out the MSB used for division per lookuptable and XOR with the remainder */
        crc = (uint)((crc << 8) ^ (uint)(crcTable__[pos]));
    }
...
uint CRC32_Fast_Gen(byte bytes[], uint len)
{
    uint crc = 0;
    for (int i = 0; i < len; ++i)
    {
...
// 按照 IEEE 802.3 描述的算法，除数为 0xEDB88320。
static const auto table = create_table(0xEDB88320);

std::uint32_t CRC32_Fast_(const void *data, std::size_t size) noexcept {
...
int main(int argc, const char * argv[]) {
    ...
    byte msg[] = { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39 };
    byte msg1[] = { 0x31, 0x32, 0x33, 0x34 };
    byte rec[] = { 0x34, 0x33, 0x32, 0x31 };
    uint ret1 = CRC32_Fast(msg, sizeof(msg));
    uint ret2 = CRC32_Slow(msg, sizeof(msg));
    uint ret3 = CRC32_Fast_Gen(msg, sizeof(msg));
    uint ret4 = CRC32_Fast__(msg, sizeof(msg));
    std::cout << "msg to be check: ";
    for (int i = 0; i < sizeof(msg); i++)
        std::cout << HEX(msg[i]);
    std::cout << std::endl;
    std::cout << "CRC32_Fast result: " << HEX(ret1) << "\n";
    std::cout << "CRC32_Slow result: " << std::hex << ret2 << "\n";
    std::cout << "impl::CRC32_Fast_ result: " << HEX(ret3) << "\n";
    std::cout << "impl::CRC32_Fast__ result: " << HEX(ret4) << "\n";


    ret1 = CRC32_Fast(msg1, sizeof(msg1));
    ret2 = CRC32_Slow(msg1, sizeof(msg1));
    ret3 = CRC32_Fast_Gen(msg1, sizeof(msg1));
    ret4 = CRC32_Fast__(msg1, sizeof(msg1));
    std::cout << "msg to be check: ";
    ...
    // print two tables:
    std::cout << "crcTable size=" << sizeof(crcTable) << std::endl;
    for (auto val : crcTable) {
        std::cout << val << " ";
    }
    std::cout << std::endl;

    std::cout << "crcTable__ size=" << sizeof(crcTable__) << std::endl;
    for (auto val : crcTable__) {
        std::cout << val << " ";
    }
    std::cout << std::endl;


    std::cout << "crcTable_ size=" << sizeof(crcTable_) << std::endl;
    for (auto val : crcTable_) {
        std::cout << val << " ";
    }
    std::cout << std::endl;
